...
public class AccountController {
    ...
    public AccountResponseDto signUp(@RequestBody @Valid final SignUpRequestDto requestDto){
        ...
        Account findAccount=accountService.findByAccountId(id);
    ...
    {
        Account findAccount = accountService.findByAccountId(accountId);
        return AccountResponseDto.from(findAccount);
    }

    @PatchMapping("/profile/{accountId}")
    @ResponseStatus(value = HttpStatus.OK)
    public AccountResponseDto update(@PathVariable final Long accountId, @RequestBody @Valid final AccountResponseDto requestDto){
